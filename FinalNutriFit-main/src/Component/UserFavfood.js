...
const UserFavfood = ({ userId }) => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const addedFoodsResponse = await axios.get(
            "http://localhost:5000/api/nutritionist/addedFoods"
          );
          setAddedFoods(addedFoodsResponse.data.data);
  
          const favFoodsResponse = await axios.get(
            `http://localhost:5000/api/user/favorites/${userId}`
          );
          setFavOfUser(favFoodsResponse.data.data);
        } catch (error) {
    ...
    const handleToggleFavorites = async (foodId) => {
      ...
      try {
        ...
        // After toggling favorites, fetch updated favorite foods
        const updatedFavFoodsResponse = await axios.get(
          `http://localhost:5000/api/user/favorites/${userId}`
        );
        setFavOfUser(updatedFavFoodsResponse.data.data);
      } catch (error) {
